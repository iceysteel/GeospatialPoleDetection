...
SAM3_PROMPTS_EXTRA = [('wooden pole', 0.25), ('power pole', 0.55)]  # (prompt, threshold)
SAM3_THRESHOLD = 0.25
...
def run_pipeline():
    ...
    for i, p in enumerate(all_points):
        ...
        # Three-tier confidence filtering
        n = len(cluster)
        if n >= 3:
            deduped.append(best)  # Strong multi-view: always keep
        elif n == 2 and best['score'] >= 0.35:
            deduped.append(best)  # Moderate multi-view with decent score
        elif n == 1 and best['score'] >= SINGLE_VIEW_MIN_SCORE:
            deduped.append(best)  # High-confidence single-view
